...
mymodel_ac <- model(label~(accelerometer_X2+accelerometer_X3+accelerometer_X4+accelerometer_X5),data=training, na.action = na.omit)
#mymodel_gr <- model(label~(gravity_X2+gravity_X3+gravity_X4+gravity_X5),data=training, na.action = na.omit)
#mymodel_gy <- model(label~(gyroscope_X2+gyroscope_X3+gyroscope_X4+gyroscope_X5),data=training, na.action = na.omit)
#mymodel_la <- model(label~(linear_acceleration_X2+linear_acceleration_X3+linear_acceleration_X4+linear_acceleration_X5),data=training, na.action = na.omit)
#mymodel_mf <- model(label~(magnetic_field_X2+magnetic_field_X3+magnetic_field_X4+magnetic_field_X5),data=training, na.action = na.omit)
#mymodel_or <- model(label~(orientation_X2+orientation_X3+orientation_X4+orientation_X5),data=training, na.action = na.omit)
#mymodel_rv <- model(label~(rotation_vector_X2+rotation_vector_X3+rotation_vector_X4+rotation_vector_X5),data=training, na.action = na.omit)
...
predicted_scores <- predicted_scores_ac + predicted_scores_la+predicted_scores_gy+predicted_scores_or+ predicted_scores_rv
library(scales)
